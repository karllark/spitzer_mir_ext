...
if __name__ == "__main__":
    ...
    parser.add_argument("--dense", help="include dense sightlines", action="store_true")
    ...
    for i, avefilename in enumerate(avefilenames):
        ...
        if plabel[i] == "diffuse":
            ...
            obsext2.read(avefilename.replace(".fits", "_FM90.fits"))
            obsext_wave = obsext2.waves["BAND"].value
            obsext_ext = obsext2.exts["BAND"]
            obsext_ext_uncs = obsext2.uncs["BAND"]

            gindxs_IRS = np.where(obsext2.npts["IRS"] > 0)
    ...
    if args.dense:
        ...
        ax[0].legend(custom_lines, ["Diffuse", "HD283809", "HD029647"])
