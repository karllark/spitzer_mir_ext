...
if __name__ == "__main__":
    ...
    parser.add_argument("--elkejk", help="plot E(lambda-K)/A(J-K)",
                        action="store_true")
    ...
    # Rieke & Lebofsky (1985)
    GL85_wave = np.array([13.0, 12.5, 12.0, 11.5, 11.0, 10.5,
                          10.0,  9.5,  9.0,  8.5,  8.0,  4.8,
                          3.5, 2.22, 1.63, 1.23])
    GL85_axav = np.array([0.027, 0.030, 0.037, 0.047, 0.060, 0.074,
    ...
    if args.elkejk:
        ...
        GL85_y = GL85_axav
    ax.plot(GL85_wave, GL85_y, 'bo',
            label='GalCenter; Rieke & Lebofsky (1985)')

    # Chiar & Tielens (2005)
    # table in A(l)/A(K) units
    CT05 = Table.read('data/pixie_dust_chiar_2005_modified.dat',
    ...
    ax.plot(CT05_wave, CT05_y, 'co',
            label='GalCenter; Chiar & Tielens (2005)')

    # Zasowski et al. 2009 (supercedes Indebetouw?)
    # for l=90
    Z09_wave = np.array([1.22, 1.63, 2.22,
                         3.6, 4.5, 5.8, 8.0])
    Z09_ehlehk_l10 = np.array([-1.97, 0.0, 1.0,
                               1.79, 1.92, 2.08, 2.00])
    Z09_ehlehk_l90 = np.array([-2.18, 0.0, 1.0,
                               1.94, 2.38, 2.78, 2.49])
    if args.elkejk:
        Z09_y_l10 = get_elkejk_from_ehlehk(Z09_wave, Z09_ehlehk_l10)
        Z09_y_l90 = get_elkejk_from_ehlehk(Z09_wave, Z09_ehlehk_l90)
        ax.plot(Z09_wave, Z09_y_l10, 'go',
                fillstyle='none', markersize=10, markeredgewidth=2.,
                label='GalPlane (l10-15); Zasowski et al. (2009)')
        ax.plot(Z09_wave, Z09_y_l90, 'go',
                markersize=10, markeredgewidth=2.,
                label='GalPlane (l90); Zasowski et al. (2009)')

    # Xue et al. 2016
    X16_wave = np.array([3.4, 4.6, 12., 22.,
                         8.23,
                         3.6, 4.5, 5.8, 8.0, 24.0,
                         1.25, 2.22])
    X16_eklejk = np.array([0.238, 0.312, 0.269, 0.370,
                           0.273,
                           0.260, 0.313, 0.355, 0.334, 0.428,
                           -1.0, 0.0])
    if args.elkejk:
        X16_y = -1*X16_eklejk
        ax.plot(X16_wave, X16_y, 'ro',
                label='GalPlane; Xue et al. (2016)')

    # Fritz et al. 2011
    F11 = Table.read('data/fritz11_galcenter.dat',
                     format='ascii.commented_header')
    F11_wave = F11['wave']
    if args.elkejk:
    ...
    if args.elkejk:
        ax.set_xlim(1.0, 40.)
        ax.set_ylim(-1.0, 1.1)
        ax.set_ylabel('$E(\lambda - K)/E(J - K)$',
