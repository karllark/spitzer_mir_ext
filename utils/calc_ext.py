...
if __name__ == "__main__":
    ...
    p92_best_params = (p92_fit.param_names, p92_fit.parameters)
    p92_per_param_vals = zip(
        p92_fit2.parameters, p92_fit2.uncs_plus, p92_fit2.uncs_minus
    )
    p92_per_params = (p92_fit2.param_names, list(p92_per_param_vals))
    ...
    fit2.plot_emcee_results(p92_fit2, filebase=out_fname.replace(".fits", ""))
    ...
    for ind in inds:
        ...
        ax.plot(1.0 / x[gindxs], model_copy(x[gindxs]), "C1", alpha=0.05)
        ax2.plot(1.0 / x[gindxs], model_copy(x[gindxs]), "C1", alpha=0.05)
    # for the figure legend
    ax.plot(1.0 / x[gindxs], model_copy(x[gindxs]), "C1", alpha=0.05, label="EMCEE Fits")
    ax2.plot(1.0 / x[gindxs], model_copy(x[gindxs]), "C1", alpha=0.05, label="EMCEE Fits")
